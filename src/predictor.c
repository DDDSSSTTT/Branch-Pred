...
init_predictor()
{
  ...
  //
   c_verbose = verbose && VERBOSE_HERE;
  if (c_verbose != 0) {
      printf ("%s\n", "----------init predicton----------");
  }
  switch(bpType) {
	...
	case TOURNAMENT:		
		...
		gmask = ~(~0<<ghistoryBits);
		if (c_verbose!=0){printf("ghistoryBits:%d,gmask:%d\n",ghistoryBits,gmask);}
	...
	case CUSTOM:
		...
		if (c_verbose!=0) {printf("pcmask:%d, de_pcmask_high:%d\n",pcmask,de_pcmask_high);}
		lmask = ~(~0<<lhistoryBits);
		gmask = ~(~0<<ghistoryBits);
...
make_prediction(uint32_t pc)
{
	...
	switch(bpType){
		...
		case TOURNAMENT:  //GSHARE + LOCAL
			...
			printf ("bhr:%d,gPredict:%d\n",bhr,global_predictor[addr_g]);
			printf ("pcIndex:%d,lPredict:%d\n",pc_index,local_predictor[addr_l]);
			printf ("choice:%d\n", selector[addr_s]);
			}
		...
		case CUSTOM:  //Adapted Gshare + LOCAL
			...
			bhr = global_history & gmask;
			pc_index = pc & pcmask; //fetch pc
			int mask_high = power(2,2*pcIndexBits) - power(2,pcIndexBits);
			int mask_low = power (2,pcIndexBits);
			de_pc_index = ((pc &de_pcmask_high)>>pcIndexBits-BIT_RESERVE)+(pc&((1<<BIT_RESERVE)-1));
			// de_pc_index = ((pc & mask_high) >> pcIndexBits) ^ (pc & mask_low);
			if (c_verbose!=0) {printf("pc_index:%d, de_pc_index:%d\n",pc_index,de_pc_index);}
			// addr_l = local_history_table[pc_index] & lmask; // fetch l history for this pc
			...
			// if ((2*result_g-1)+(2*result_l-1)+(2*de_result_l-1)>0){	
				result = TAKEN;
			}
			else{
				result = NOTTAKEN;
...
train_predictor(uint32_t pc, uint8_t outcome)
{
  ...
  if ((bpType==GSHARE) || (bpType==TOURNAMENT)||(bpType==CUSTOM)){
	...
	addr_g = (bpType==CUSTOM)?bhr ^ pc_index:bhr;
	...
	if ((bpType==TOURNAMENT)||(bpType==CUSTOM)){
		...
		if (c_verbose!=0){printf("Tickets for g: %d l: %d d: %d",ticket_g,ticket_l,ticket_d);}
		}
	...
	// } 
	// if (c_verbose!=0){printf("Updating global_predictor[%d]\n",addr_g);}
   ...
   if (bpType==CUSTOM){ //Must check
	...
	if(bpType==CUSTOM){
		...
		// if (c_verbose!=0){printf("choice now changed to %d\n",choice[pc_index]);}
	}
